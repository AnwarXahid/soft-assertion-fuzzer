...
"""
Source: https://github.com/longyahui/GCNMDA/blob/master/src/crf.py
"""
...
def load_data(train_arr, test_arr):
    ...
    features = np.vstack((np.hstack((F1, np.zeros(shape=(1373, 173), dtype=int))),
                          np.hstack((np.zeros(shape=(173, 1373), dtype=int), F2))))
...
def adj_to_bias(sizes=[1546], nhood=1):
    ...
    M = sp.csr_matrix((labels[train_arr, 2], (labels[train_arr, 0] - 1, labels[train_arr, 1] - 1)),
                      shape=(1373, 173)).toarray()
    adj = np.vstack((np.hstack((np.zeros(shape=(1373, 1373), dtype=int), M)),
                     np.hstack((M.transpose(), np.zeros(shape=(173, 173), dtype=int)))))
